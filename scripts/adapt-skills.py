#!/usr/bin/env python3
"""Normalize imported skill metadata and platform vocabulary for OpenCode."""
...
replacements = {
    ...
    "`create-skill`'s YAML rules": "the OpenCode skill format at https://opencode.ai/docs/skills",
    ...
    "the **create-skill** skill (Cursor's built-in for authoring SKILL.md files)": "the authoring-a-skill playbook and the OpenCode skill format (https://opencode.ai/docs/skills)",
    ...
    "One Cursor cloud agent per PR": "One OpenCode subagent in an exclusive worktree per PR",
    "each a Cursor cloud agent": "each an OpenCode subagent in an exclusive worktree",
...
for path in (root / "skills").rglob("*.md"):
    ...
    if path.name == "SKILL.md":
        ...
        text = re.sub(r"^(?:disable-model-invocation|mode|icon|color|reminder|paths):.*\n", "", text, flags=re.M)
    text = text.replace("~/.cursor/skills/", "~/.config/opencode/skills/")
    text = text.replace(".cursor/skills/", ".opencode/skills/")
    text = text.replace("AskQuestion", "question")
    text = text.replace("Cursor restart", "OpenCode restart")
...
for relative in ("reflect/SKILL.md", "show-me-your-work/SKILL.md", "poteto-mode/playbooks/eval.md", "poteto-mode/playbooks/session-pickup.md"):
    ...
    text = path.read_text()
    text = re.sub(r"The parent finds its own transcript file.*?unrelated projects\.", "The parent locates this project's session with `opencode session list` and exports it with `opencode export <sessionID>`. Confirm the project and opening user message match before reading it. Do not inspect other projects' sessions.", text)
    text = re.sub(r"Read this run's transcript under.*?unrelated private chats\.", "Locate this project's session with `opencode session list` and read `opencode export <sessionID>`. Confirm its project and opening prompt before using it.", text)
    text = re.sub(r"Read each candidate's local transcript under.*?unrelated projects\.", "Export each candidate's session with `opencode export <sessionID>`. Check that it belongs to this project and evaluation before reading it.", text)
    text = re.sub(r"A local transcript under.*?unrelated projects\), a cloud-agent URL, or a pushed branch\.", "Use `opencode session list` and `opencode export <sessionID>` for this project's prior session, a supplied session export, or a pushed branch.", text)
    if text != path.read_text():
        path.write_text(text)
