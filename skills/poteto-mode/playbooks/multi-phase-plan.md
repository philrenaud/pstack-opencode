...
4. Copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write the file under `.pstack/state/docs/` in the target project. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (the **sequence-verifiable-units** principle skill). Name the execution playbook in **How to read this**. Pick between `playbooks/autopilot-full.md` and `playbooks/autopilot-stack.md` per the rule at the end of `playbooks/autopilot-stack.md`. A standing program takes `playbooks/orchestrate.md`.
...
- [ ] Record a 30-minute audit deadline in the run state and check it between tool calls and worker completions. Checkpoint before the session ends, per `../references/runtime.md`.
