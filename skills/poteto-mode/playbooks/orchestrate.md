...
Size the brief to the unit. A one-command unit gets the template collapsed to a paragraph that still names goal, scope, the verify command, and the report shape. Local spawns may reference the standing-orders file by absolute path. Every resume includes them verbatim.

A sub-coordinator brief adds its track boundary and unit list, its spawn budget and permitted nesting, the drain protocol, and the rollup format (per child: name, status, PR, head SHA, verdict, one line, plus track status and frontier delta).
...
- Drain in batches at four points: the end of a critical section, a track rollup, a bounded frontier watcher result, and before a human report. Check the recorded audit deadline between calls. Begin each batch with `orch inbox drain`. Arrivals during a drain wait for the next one.
...
- Exactly one stacker per stack may run `gt`, serialized within its stack. Record the holder and its exclusive checkout in the standing orders. Budget local disk and CPU before a large restack.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only through the ledger, `units.tsv`, `gh`, pushed branches, and available session status. Transcript mtime is not liveness.
