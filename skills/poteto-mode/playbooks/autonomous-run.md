...
2. Pick the next observation. For CI, a merge, or a ref advancing, run a bounded watcher call. Otherwise iterate and check the done predicate at useful intervals. Follow `../references/opencode-runtime.md`. Persist the next action before exiting. OpenCode does not provide a durable wake scheduler.
