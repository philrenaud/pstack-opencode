...
- Shipping UI / IDE / CLI → the project-local `verify-*` skill. Generate one with **create-verification-skill** when needed. Playbooks call this the control skill. For bug fixes, reproduce first on the same surface yourself. Hand to the user only under the narrow Bug fix step 1 exception.
