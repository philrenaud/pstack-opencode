...
- Prefer `subagent_type: "poteto-agent"`. The built-in `general` is the fallback, and the built-in `explore` works for pure file/keyword searches. Never use the built-in `plan` agent for this; it ignores this skill.
- Route by role: `poteto-grok` for code exploration, `poteto-claude` for judgment (models are pinned in the pstack repo's `agents/` directory).
...
If a phase creates or edits a skill, the phase instructs the implementer to follow the authoring-a-skill playbook and the OpenCode skill format (https://opencode.ai/docs/skills).
...
**Runtime.** Exercise the feature on the matching surface via the project-local verify skill:

- The project has a `verify-*` skill: use it.
- No verify skill for the touched surface: generate one with **create-verification-skill**, or flag the gap in the plan.
...
- the **unslop** skill's rules over each diff before commit and over any prose surface.
- the **show-me-your-work** skill to keep a decision trail when the plan is large enough to need an auditable record.
- watching the PR after opening it: `gh pr checks <number> --watch`, then triage comments.
