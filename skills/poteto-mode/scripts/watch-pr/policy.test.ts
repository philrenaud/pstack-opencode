...
describe("readiness truth table", () => {
  ...
  it("never classifies explicit review-required or unresolved merge state as ready", async () => {
    for (const facts of [
  ...
  it("allows a clean PR with no repository review requirement", async () => {
    const snapshot = await readSnapshot({
...
it("caps a long polling interval at the deadline without another query", async () => {
  let now = 0;
  const sleeps: number[] = [];
  const reader = fakeReader({
    fastPath: { kind: "checks", checks: [pendingCheck()] },
  ...
  const verdict = await runSimple({
    ...
    mode: "single",
    statusOnly: false,
    options: { ...options, interval: 60, timeout: 1 },
  });
