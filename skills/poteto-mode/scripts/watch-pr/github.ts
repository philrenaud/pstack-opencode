...
function run(
  ...
  return new Promise((resolve, reject) => {
    const remainingMilliseconds =
      remaining === null ? null : Math.max(0, remaining() * 1_000);
    if (remainingMilliseconds === 0) {
      reject(
        new WatcherQueryError({
          kind: "command-timeout",
          retryable: true,
          detail: `${argv.join(" ")} exceeded watcher deadline`,
        })
      );
      return;
    }
...
async function runJson(
  ...
  const result = await run(argv, remaining);
  if (result.code !== 0)
    throw new WatcherQueryError({
      kind: "command-exit",
      retryable: true,
...
export class GhGitHubReader implements T.GitHubReader {
  ...
  constructor(command = "gh") {
    this.#command = command;
  }
  setDeadline(deadline: number | null, now: () => number): void {
    this.#deadline = deadline === null ? null : () => deadline - now();
  }
  ...
  async currentPr(pr: T.PrNumber | null): Promise<T.PrContext> {
    const argv: [string, ...string[]] = [this.#command, "pr", "view"];
    if (pr !== null) argv.push(String(pr));
    argv.push("--json", "number,url");
    const object = record(await runJson(argv, this.#deadline), "current PR");
  ...
  async checkRollupPage(
    ...
    const argv = graphqlArgs(this.#command, PR_CHECK_ROLLUP_QUERY, context);
    if (after !== null) argv.push("-f", `after=${after}`);
    const value = await runJson(argv, this.#deadline);
    const commits = list(
  ...
  async reviewThreads(
    ...
    do {
      const argv = graphqlArgs(this.#command, REVIEW_THREADS_QUERY, context);
      if (after !== null) argv.push("-f", `after=${after}`);
      const value = await runJson(argv, this.#deadline);
      ...
      const pageInfo = record(reviewThreads.pageInfo, "reviewThreads.pageInfo");
      ...
      if (!pageInfo.hasNextPage) {
        ...
        if (cursor === after || seen.has(cursor))
          missing("reviewThreads.pageInfo.endCursor progress", cursor);
        seen.add(cursor);
        after = cursor;
      }
    } while (after !== null);
  ...
  async commitRollups(
    ...
    const value = await runJson(
      graphqlArgs(this.#command, PR_COMMIT_STATUS_QUERY, context),
      this.#deadline
    );
    const commits = list(
      at(value, ["data", "repository", "pullRequest", "commits", "nodes"]),
