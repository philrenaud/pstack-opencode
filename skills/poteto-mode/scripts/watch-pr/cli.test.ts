...
describe("main", () => {
  ...
  it("bounds a hung gh call during context discovery", async () => {
    const directory = await mkdtemp(join(tmpdir(), "watch-pr-hung-gh-"));
    const command = join(directory, "gh");
    await writeFile(command, "#!/bin/sh\nexec sleep 30\n");
    await chmod(command, 0o755);
    const output: string[] = [];
    const started = performance.now();
    const code = await main(["--timeout", "1"], {
      reader: new GhGitHubReader(command),
      clock: {
        now: () => Date.now() / 1_000,
