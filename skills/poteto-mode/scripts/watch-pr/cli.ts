...
export async function main(
  ...
  const deadline =
    options.polling.timeout > 0
      ? runtime.clock.now() + options.polling.timeout
      : null;
  if (runtime.reader instanceof GhGitHubReader)
    runtime.reader.setDeadline(deadline, () => runtime.clock.now());
  try {
    ...
    if (deadline !== null && runtime.clock.now() >= deadline)
      throw new WatcherQueryError({
        kind: "command-timeout",
        retryable: true,
        detail: "watcher deadline elapsed during PR discovery",
      });
  } catch (error) {
    if (!(error instanceof WatcherQueryError)) throw error;
    const stamp = verdictFactory(runtime.clock, options.mode);
    const verdict = error.failure.kind === "command-timeout"
      ? stamp({
          kind: "TIMEOUT",
          terminal: true,
          exitCode: 5,
