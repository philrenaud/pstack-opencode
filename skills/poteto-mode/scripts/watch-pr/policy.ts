...
export function assessGitHubMerge(args: {
  ...
  if (
    ...
    args.mergeStateStatus === "UNKNOWN" ||
    args.mergeStateStatus === "BEHIND"
  ) {
    return {
      kind: "refused",
...
async function pollUntilTerminal<V>(args: {
  ...
  const deadline =
    ...
    (args.options.timeout > 0 ? started + args.options.timeout : null);
  const remaining = (): number | null =>
    deadline === null
      ? null
      : Math.max(0, deadline - args.dependencies.clock.now());
  let onDeadline = args.onDeadline;
  while (true) {
    ...
    try {
      ...
      await args.dependencies.clock.sleep(
        retryRemaining === null
          ? retryInSeconds
          : Math.min(retryInSeconds, retryRemaining)
      );
      if (remaining() === 0)
        return args.stamp({
          kind: "TIMEOUT",
    ...
    if (result.kind === "sleep") {
      if (result.onDeadline !== undefined) onDeadline = result.onDeadline;
      if (
        result.onDeadline !== undefined &&
        remaining() === 0
      )
        return onDeadline();
      const sleepRemaining = remaining();
      await args.dependencies.clock.sleep(
        sleepRemaining === null
          ? result.seconds
          : Math.min(result.seconds, sleepRemaining)
      );
      if (remaining() === 0) return onDeadline();
    }
  }
      ...
      stamp({
        ...
        reason: {
          ...
          failure: {
            ...
            retryable: true,
            detail: "watcher deadline elapsed before PR status was available",
          },
        },
      }),
  });
  ...
  return pollUntilTerminal({
    ...
    onDeadline: () =>
      stamp({
        ...
        reason: {
          kind: "queued-stack",
          frontier: state.frontier ?? state.queue[0],
          unmergedCount: activeRows(state).length || state.queue.length,
        },
      }),
  });
}
