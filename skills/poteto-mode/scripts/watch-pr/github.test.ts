...
describe("review thread pagination", () => {
  it("keeps an unresolved second-page thread and counts Bugbot passes across pages", async () => {
        ...
        nodes: [
          {
            id: "resolved",
            isResolved: true,
            comments: { nodes: [comment("RUN_ID: first")] },
          },
        ],
        hasNextPage: true,
        endCursor: "next",
        ...
        nodes: [
          {
            id: "unresolved",
            isResolved: false,
            comments: { nodes: [comment("RUN_ID: second")] },
          },
        ],
        hasNextPage: false,
        endCursor: null,
    ...
    const command = await fakeGh(
      `case "$*" in *after=next*) printf '%s' '${second}' ;; *) printf '%s' '${first}' ;; esac`
    );
    const threads = await new GhGitHubReader(command).reviewThreads(context);
    expect(threads).toMatchObject([
      { id: "unresolved", bugbotReviewPasses: 2 },
    ]);
  });

  it("rejects missing and repeated cursors", async () => {
    for (const cursor of [null, "same"]) {
      const page = JSON.stringify(
        threadPage({ nodes: [], hasNextPage: true, endCursor: cursor })
      );
      const command = await fakeGh(`printf '%s' '${page}'`);
      await expect(
        new GhGitHubReader(command).reviewThreads(context)
