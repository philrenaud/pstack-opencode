...
for path in Path(sys.argv[1]).rglob("*"):
    ...
    if sys.argv[2] + "/" in text or sys.argv[2] + '"' in text:
        latest = max(latest, int(path.stat().st_mtime))
print(latest)
PY
		)
		if [ "$last_ts" -gt 0 ]; then last=$(date -r "$last_ts" '+%Y-%m-%d' 2>/dev/null); fi
	fi
	...
	case "$dirty" in wip:*) bucket=hold-wip ;; *)
		case "$pr" in *OPEN*) bucket=hold-open-pr ;; *)
			...
			elif [ "$dirty" != clean ]; then bucket=hold-scratch
			elif [ "$merged" = YES ] || [[ "$pr" == */MERGED ]]; then bucket=verify-session
