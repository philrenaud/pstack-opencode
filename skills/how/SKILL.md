...
- `subagent_type`: `poteto-grok`
- each prompt includes: review only, do not edit files
...
- the prompt includes: review only, do not edit files
...
After the explanation is complete, spawn one Task per panelist agent: `poteto-claude`, `poteto-gpt`, `poteto-grok`, all in a single message.

For each critic:
- `subagent_type`: one panelist agent (`poteto-claude`, `poteto-gpt`, or `poteto-grok`). Model choices are configured per agent in this repo's `agents/` directory.
- the prompt includes: review only, do not edit files
