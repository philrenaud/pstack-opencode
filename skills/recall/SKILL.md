...
3. Fan out across your chat history with `poteto-grok`, each taking a slice of the corpus. Order candidates by `opencode session list` recency and export matches with `opencode export <sessionID>`. Search the topic first, then read only matching chats and their relevant regions. Skip the current chat plus obvious noise from subagent, eval, and test sessions. Each returns one block per chat with topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts such as PRs, tickets, and branches. Cite each session ID. For one or two chats, search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
